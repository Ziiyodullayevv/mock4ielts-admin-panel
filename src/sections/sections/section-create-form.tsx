...
export function SectionCreateForm() {
  ...
  // Auto-generate parts when section type changes
  useEffect(() => {
    if (!watchedSectionType) return;
    const partConfig = SECTION_PART_COUNTS[watchedSectionType];
    if (!partConfig) return;

    const newParts: PartFormValues[] = partConfig.labels.map((label) => ({
      title: label,
      instructions: '',
      passage_text: '',
      audio_url: '',
          ...
          <Stack spacing={2}>
            {!watchedSectionType && (
              <Card sx={{ p: 5, textAlign: 'center' }}>
                <Typography color="text.disabled">Select a section type to begin</Typography>
              </Card>
            )}
